// Shared helpers for the plain-string bullet convention used across the importer, AI
// normalization, and rendering. A "bullet list" is a plain string where each bullet line
// is prefixed with BULLET_MARKER ("• "). Keep this file in sync with
// frontend/src/app/integration/bulletText.ts.
...
// True when the string contains at least one bullet marker line (cheap raw-string check).
export const containsBulletLines = (text: string): boolean => {
  if (!text.includes("\n") && !isBulletLine(text)) {
    return false;
  }
